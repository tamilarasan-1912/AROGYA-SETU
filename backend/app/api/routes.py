...
router = APIRouter()
# Public/API language code is the canonical application code. Sindhi is `sd`;
# model adapters translate it to the Indic model's `snd` code.
...
async def audio_pipeline_endpoint(
    ...
    result = run_clinical_decision_pipeline(asr["transcript"], language, patient_id, {}, [], location, specialty)
    result["asr"] = asr
